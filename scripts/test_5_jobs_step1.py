...
def monitor_job(task_info: Dict, event_log: List[Dict]) -> Dict:
    ...
    if result["status"] == "completed":
        ...
        print(f"✅ Job {job_index} completed: {result['status']} ({int(total_time)}s, {events_count} WebSocket events)")
    else:
        print(f"❌ Job {job_index} {result['status']}: {result.get('error', 'Unknown error')}")
...
def main():
    ...
    print("=" * 85)
    print(f"🧪 Test: 5 Jobs หลังจาก Step 1 (Worker Configuration)")
    print("=" * 85)
    print()
