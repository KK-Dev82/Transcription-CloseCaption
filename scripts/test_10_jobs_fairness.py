#!/usr/bin/env python3
"""
Test Script: ทดสอบ 5 Jobs หลังจาก Step 1 (Worker Configuration)
- Step 1: Worker Configuration
  - GPU_WORKERS_PER_GPU: 8 → 3
  - NUM_PREPROCESS_WORKERS: 6 → 2
  - NUM_CPU_WORKERS: 4 → 2
- Submit 10 jobs
...
async def listen_websocket_events(task_ids: List[str], user_id: str, event_log: List[Dict], timeout: int = 600):
    """Listen to WebSocket events for given task IDs"""
    if not WEBSOCKETS_AVAILABLE:
        return
    
    try:
        ws_url = f"{WS_BASE_URL}/ws/transcription/{user_id}"
                    ...
                    try:
                        ...
                        try:
                            ...
                            if task_id and task_id in task_ids:
                                ...
                                print(f"    📡 WebSocket: {event_type} for {task_id[:8]}... - Progress: {progress}%, Stage: {stage}")
...
def wait_for_completion(task_id: str, event_log: List[Dict], timeout: int = 3600) -> Dict:
    """Wait for task to complete and track WebSocket events"""
    start_time = time.time()
    last_status = None
    last_progress = 0
    events_received = []
...
def monitor_job(task_info: Dict, event_log: List[Dict]) -> Dict:
    ...
    result = wait_for_completion(task_id, event_log)
    result["job_index"] = job_index
    
    if result["status"] == "completed":
        total_time = result.get("total_time") or result.get("elapsed_time", 0)
        events_count = result.get("events_received", 0)
        print(f"✅ Job {job_index} transcription completed: {result['status']} ({int(total_time)}s, {events_count} WebSocket events)")
    else:
        print(f"❌ Job {job_index} transcription {result['status']}: {result.get('error', 'Unknown error')}")
...
def main():
    ...
    print("=" * 85)
    print(f"🧪 Test: 10 Jobs หลังจาก Step 1 (Worker Configuration)")
    print("=" * 85)
    print()
    ...
    print(f"  GPU_WORKERS_PER_GPU: {os.getenv('GPU_WORKERS_PER_GPU', '3')}")
    print(f"  NUM_PREPROCESS_WORKERS: {os.getenv('NUM_PREPROCESS_WORKERS', '2')}")
    print(f"  NUM_CPU_WORKERS: {os.getenv('NUM_CPU_WORKERS', '2')}")
    print(f"  MAX_PREPROCESS_QUEUE_SIZE: {os.getenv('MAX_PREPROCESS_QUEUE_SIZE', '25')}")
    print(f"  WebSocket Support: {'✅ Available' if WEBSOCKETS_AVAILABLE else '❌ Not available (pip install websockets)'}")
    print()
    ...
    submit_elapsed = time.time() - overall_start_time
    print()
    ...
    queue_full_jobs = [r for r in submitted_jobs if r["status"] == "queue_full"]
    error_jobs = [r for r in submitted_jobs if r["status"] not in ["queued", "queue_full"]]
    ...
    # Phase 2: Monitor jobs with WebSocket events
    print("=" * 85)
    print(f"Step 2: Monitor {len(queued_jobs)} jobs until completion (with WebSocket)")
    print("=" * 85)
    print()
    ...
    if WEBSOCKETS_AVAILABLE:
        try:
            ...
            ws_task = loop.create_task(listen_websocket_events(task_ids, user_id, event_log, timeout=600))
    ...
    with ThreadPoolExecutor(max_workers=len(queued_jobs)) as executor:
        futures = {
            executor.submit(monitor_job, job, event_log): job["job_index"]
    ...
    print(f"  Transcription Completed: {len(completed_jobs)} jobs")
    ...
    if len(event_log) > 0:
        ...
        for event in event_log:
            event_data = event.get("data", {})
            event_type = event_data.get("type", "unknown")
            if event_type != "ping":  # Filter out ping messages
        ...
        if event_types:
            ...
            for event in event_log:
                event_data = event.get("data", {})
                event_type = event_data.get("type", "unknown")
                if event_type != "ping" and sample_count < 5:
                    task_id = event_data.get("task_id", "")[:8] if event_data.get("task_id") else "N/A"
                    progress = event_data.get("progress", 0)
                    stage = event_data.get("stage", "")
                    print(f"    {sample_count + 1}. {event_type} (task: {task_id}..., progress: {progress}%, stage: {stage})")
                    sample_count += 1
            print()
        else:
            print("  ⚠️  No events received (only ping messages)")
            print()
    else:
    ...
    if len(completed_jobs) > 0:
        elapsed_times = [r.get("total_time") or r.get("elapsed_time", 0) for r in completed_jobs]
        total_time = monitor_elapsed
        min_time = min(elapsed_times)
        max_time = max(elapsed_times)
        avg_time = sum(elapsed_times) / len(elapsed_times)
        throughput = len(completed_jobs) / (total_time / 60) if total_time > 0 else 0
        ...
        print(f"{'Throughput':<40} {throughput:.2f} jobs/min")
        print()
        ...
        print("=" * 85)
        print()
        for job in sorted(completed_jobs, key=lambda x: x["job_index"]):
            job_time = job.get("total_time") or job.get("elapsed_time", 0)
            print(f"  Job {job['job_index']}: {job_time:.1f}s ({job_time/60:.2f} min)")
        print()
